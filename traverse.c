                  ...
                  struct sync_data *sd)
{
	...
	if (tmp == NULL)
		goto err0;
	sd->src = tmp;
	snprintf(sd->src, buf_size, "%s", src);
	...
	if (tmp == NULL)
		goto err1;
	sd->dst = tmp;
	snprintf(sd->dst, buf_size, "%s/%s", dst, suffix);
 ...
 *
 * ${src_paths} and ${dst_path} must be absolute paths as fts is not initialized
 * with FTS_NOCHDIR.
 *
...
traverse_and_queue(char *src_paths[], char *dst_path, struct sync_data_mpmc_queue *Q)
{
	...
	errno = 0;
	FTS *fts = fts_open(src_paths, FTS_NOSTAT | FTS_PHYSICAL, NULL);
	if (fts == NULL) {
	...
	while ((ftsent = fts_read(fts)) != NULL) {
		...
		case FTS_D:
			...
			snprintf(dst_dir_buf, dst_dir_buf_len, "%s/%s", dst_path, suffix);
