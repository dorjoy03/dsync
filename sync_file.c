...
sync_file(char *src, char *dst, bool force_copy)
{
	...
	case S_IFLNK:
		if ((uintmax_t) src_statbuf.st_size > (uintmax_t) SIZE_MAX) {
			fprintf(stderr, "Skipping copy of symbolic link %s. Too large\n", src);
			goto err0;
		}

		ret = copy_symlink(src, dst, src_statbuf.st_size);
		if (ret != 0) {
			fprintf(stderr, "Failed to copy symbolic link %s to %s\n", src, dst);
			goto err0;
		}
		break;
	...
	case S_IFREG:
		...
		if (ret != 0) {
			fprintf(stderr, "Failed to copy of %s to %s\n", src, dst);
			goto err0;
		}
		break;
